...
/*
VALUE = correct
need to check keys and values with lengths more than max
*/

#define VALUE_INV	"iiiiiiiiiiiiiiiiiiiiiii"
#define KEY_INV		"iiiiiiiiiiiiiiiiiiiiiii"

/* What's being checked in this file?
1. valid inputs for 1 and 2 tables
2. Invalid inputs
*/
 ...
 * error).
 */
START_TEST (test_get1tb)/* correct input values*/
{
	// Start the server.
...
/*	fail_if(conn == NULL, "Couldn't connect to server.");*/
	...
	// Disconnect from the server silently
 ...
 */
START_TEST (test_get2tb) /* correct input values*/
{
	// Start the server.
...
/*	fail_if(conn == NULL, "Couldn't connect to server.");*/
	...
	// Set a key/value in the second table silently.
	struct storage_record r;
	strncpy(r.value, VALUE, sizeof r.value);
	storage_set(TABLE2NAME, KEY, &r, conn);
...
START_TEST (test_getkinv)/* correct input values*/
{
	// Start the server.
	int serverpid = start_server(ONETABLE_CONF); fail_if(serverpid <= 0,
			"Couldn't start server.");
...
/*	fail_if(conn == NULL, "Couldn't connect to server.");*/
	...
	struct storage_record r;
	strncpy(r.value, VALUE, sizeof r.value);
	storage_set(TABLE1NAME, KEY_INV, &r, conn);

	// Get a value.
	strncpy(r.value, "", sizeof r.value);
	status = storage_get(TABLE1NAME, KEY_INV, &r, conn);
	fail_if(status != 0, "Error getting a value.");
	fail_if(strcmp(r.value, VALUE) != 0, "Got wrong value.");
...
START_TEST (test_getvinv)/* correct input values*/
{
	// Start the server.
	int serverpid = start_server(ONETABLE_CONF); fail_if(serverpid <= 0,
			"Couldn't start server.");
	...
	// Set a key/value (silently)
	struct storage_record r;
	strncpy(r.value, VALUE_INV, sizeof r.value);
	storage_set(TABLE1NAME, KEY, &r, conn);
	...
	status = storage_get(TABLE1NAME, KEY, &r, conn);
	fail_if(status != 0, "Error getting a value.");
	fail_if(strcmp(r.value, VALUE_INV) != 0, "Got wrong value.");
...
int main(int argc, char *argv[])
{
	...
	tcase_add_test(tc, test_get1tb);
	tcase_add_test(tc, test_get2tb);
	tcase_add_test(tc, test_getkinv);
	tcase_add_test(tc, test_getvinv);
