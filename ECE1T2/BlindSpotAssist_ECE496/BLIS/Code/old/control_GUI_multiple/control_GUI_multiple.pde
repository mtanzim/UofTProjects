...
void draw() {
  ...
  get_serial_data();
  println(speed);
  text ("Speed:", h_offset+EDGE - 115, 25);
  text (speed,h_offset+EDGE - 65 ,25);
  
  text ("Distance:", h_offset+EDGE - 115, 15);
  text (distance, h_offset+EDGE - 65 ,15);
  
  text ("Time:", h_offset+EDGE - 115, 35);
  text (time, h_offset+EDGE - 65 ,35);
